 ...
 * MDataFrame::
 *
 * A MDataFrame is table composed of columns of equal length. Each column
 * holds values of the same type (I, S, B, F). A mDataFrame has a schema that
 * describes it.
 */
class MDataFrame : public Object {
public:
    ...
    MDataFrame(MDataFrame &df) : MDataFrame(*df.schema) {
...
//        Iterate through each of the elements in the row and give them to Fielder
    ...
    /** Returns the mDataFrame's schema-> Modifying the schema after a mDataFrame
      * has been created in undefined. */
    Schema &get_schema() {
        return *this->schema;
    }

    /** Adds a column this mDataFrame, updates the schema, the new column
      * is external, and appears as the last column of the mDataFrame, the
    ...
    /** Set the fields of the given row object with values from the columns at
      ...
      * mDataFrame, results are undefined.
      */
...
//        std::cout << idx << std::endl;
    ...
    /** Add a row at the end of this mDataFrame. The row is expected to have
    ...
    /** The number of rows in the mDataFrame. */
    size_t nrows() {
        return this->find_max_len_of_columns_();
    }

    /** The number of columns in the mDataFrame.*/
    size_t ncols() {
    ...
    void pmap(Rower &r) {
        ...
        std::thread t1(&MDataFrame::map_chunk_, this, std::ref(r), start, end);
        start = end;
        end += chunkSize;
        std::thread t2(&MDataFrame::map_chunk_, this, std::ref(*r2), start, end);
        start = end;
        end += chunkSize;
        std::thread t3(&MDataFrame::map_chunk_, this, std::ref(*r3), start, end);
        start = end;
        end = nrows();
        std::thread t4(&MDataFrame::map_chunk_, this, std::ref(*r4), start, end);
        t1.join();
        t2.join();
        t3.join();
        t4.join();
    ...
    /** Create a new mDataFrame, constructed from rows for which the given Rower
      * returned true from its accept method. */
    MDataFrame *filter(Rower &r) {
        MDataFrame *filtered_MDataFrame = new MDataFrame(*this->schema);
        for (int row_idx = 0; row_idx < this->nrows(); row_idx++) {
            ...
            if (r.accept(*curr_row)) {
                filtered_MDataFrame->add_row(*curr_row);
            }
            delete curr_row;
        }
        return filtered_MDataFrame;
    }
